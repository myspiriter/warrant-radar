...
APP_VERSION = "V6.21｜PRO Dashboard"


# V6.21 PRO Dashboard theme — uses explicit HTML cards plus Streamlit theming.
st.markdown(r"""
<style>
.stApp { background: linear-gradient(135deg,#07111f 0%,#0b1728 48%,#101d30 100%); color:#e8eef8; }
[data-testid="stSidebar"] { background:#081321; border-right:1px solid rgba(201,169,98,.18); }
[data-testid="stHeader"] { background:rgba(7,17,31,.82); }
.block-container { max-width:1500px; padding-top:1.35rem; }
h1,h2,h3 { color:#f4f7fb !important; letter-spacing:-.02em; }
p, label, .stCaption { color:#aebbd0 !important; }
[data-testid="stMetric"] { background:linear-gradient(145deg,rgba(19,34,55,.96),rgba(10,23,40,.96)); border:1px solid rgba(201,169,98,.18); border-radius:18px; padding:16px 18px; box-shadow:0 12px 32px rgba(0,0,0,.18); }
[data-testid="stMetricLabel"] { color:#92a3bb !important; }
[data-testid="stMetricValue"] { color:#f7f3e8 !important; font-weight:750; }
[data-baseweb="tab-list"] { background:#0a1626; padding:6px; border-radius:14px; gap:4px; border:1px solid rgba(255,255,255,.06); }
[data-baseweb="tab"] { border-radius:10px; color:#9eacc0; }
[aria-selected="true"] { background:linear-gradient(135deg,#203654,#172943) !important; color:#f4d68c !important; }
[data-testid="stDataFrame"] { border:1px solid rgba(255,255,255,.07); border-radius:16px; overflow:hidden; }
.stButton>button { border-radius:12px; border:1px solid rgba(201,169,98,.30); background:#13243a; color:#f4d68c; }
[data-testid="stExpander"] { background:rgba(13,28,47,.72); border:1px solid rgba(255,255,255,.07); border-radius:14px; }
.pro-hero { padding:26px 30px; margin:0 0 18px; border-radius:24px; background:radial-gradient(circle at 85% 10%,rgba(201,169,98,.16),transparent 34%),linear-gradient(135deg,#122741,#091728); border:1px solid rgba(201,169,98,.24); box-shadow:0 20px 55px rgba(0,0,0,.28); }
.pro-kicker { color:#d6b86f; font-size:.78rem; font-weight:800; letter-spacing:.18em; text-transform:uppercase; }
.pro-title { color:#f8fafc; font-size:2.15rem; line-height:1.1; font-weight:820; margin:7px 0 9px; }
.pro-sub { color:#9fb0c7; font-size:.95rem; }
.pro-badge { display:inline-block; margin-top:14px; padding:7px 11px; border-radius:999px; background:rgba(78,211,159,.10); border:1px solid rgba(78,211,159,.24); color:#79e2b7; font-size:.78rem; font-weight:700; }
</style>
...
cfg = load_cfg()
st.markdown(f"""<div class="pro-hero"><div class="pro-kicker">AI MARKET INTELLIGENCE · TWSE</div><div class="pro-title">AI PRO 台股權證雷達</div><div class="pro-sub">全市場策略掃描 · 盤中行情覆蓋 · 趨勢 / 量價 / 事件 / 權證決策儀表板</div><div class="pro-badge">● SYSTEM ONLINE &nbsp; {APP_VERSION}</div></div>""", unsafe_allow_html=True)
st.caption("策略底稿使用 TWSE 公開資料；盤中行情以 TWSE 市況資訊做最佳努力覆蓋，實際下單前仍請以券商即時報價確認。")
