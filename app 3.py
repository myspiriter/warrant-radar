...
st.set_page_config(page_title="WARRANT RADAR PRO", page_icon="📡", layout="wide")
APP_VERSION = "V6.20 PRO｜高級介面版"

st.markdown(r"""
<style>
:root {
  --bg:#07101c; --panel:#0d1826; --panel2:#101e2f; --line:rgba(255,255,255,.09);
  --text:#eef4fb; --muted:#91a0b4; --gold:#d5ad5d; --cyan:#5fd2ff; --green:#5bd6a2; --red:#ff7a86;
}
.stApp {
  background:
    radial-gradient(circle at 16% -10%, rgba(95,210,255,.11), transparent 28%),
    radial-gradient(circle at 90% 0%, rgba(213,173,93,.10), transparent 25%),
    linear-gradient(180deg,#07101c 0%,#091422 48%,#07101c 100%);
  color:var(--text);
}
[data-testid="stHeader"] {background:rgba(7,16,28,.84); backdrop-filter: blur(12px); border-bottom:1px solid var(--line);}
[data-testid="stSidebar"] {background:linear-gradient(180deg,#091421,#0b1726); border-right:1px solid var(--line);}
[data-testid="stSidebar"] * {color:#dbe6f4;}
.block-container {padding-top:1.55rem; max-width:1600px;}
h1,h2,h3 {letter-spacing:-.02em;}
h2,h3 {color:#f5f8fc !important;}
.pro-hero {
  position:relative; overflow:hidden; padding:26px 30px; margin:0 0 18px 0;
  border:1px solid rgba(213,173,93,.25); border-radius:22px;
  background:linear-gradient(120deg,rgba(16,30,47,.98),rgba(8,20,34,.96));
  box-shadow:0 18px 55px rgba(0,0,0,.35), inset 0 1px 0 rgba(255,255,255,.04);
}
.pro-hero:after {content:""; position:absolute; right:-90px; top:-120px; width:360px; height:360px; border-radius:50%; background:radial-gradient(circle,rgba(95,210,255,.18),transparent 66%);}
.pro-kicker {font-size:12px; letter-spacing:.22em; color:var(--gold); font-weight:700; text-transform:uppercase;}
.pro-title {font-size:34px; line-height:1.1; font-weight:800; margin-top:7px; color:white;}
.pro-sub {font-size:14px; color:var(--muted); margin-top:9px;}
.pro-badge {display:inline-block; margin-top:13px; padding:6px 10px; border:1px solid rgba(95,210,255,.26); border-radius:999px; color:#9fe5ff; background:rgba(95,210,255,.07); font-size:12px;}
.kpi-grid {display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin:14px 0 18px;}
.kpi-card {padding:17px 18px;border:1px solid var(--line);border-radius:16px;background:linear-gradient(145deg,rgba(16,30,47,.94),rgba(10,22,37,.94));box-shadow:0 10px 30px rgba(0,0,0,.23);}
.kpi-label {font-size:12px;color:var(--muted);letter-spacing:.08em;}
.kpi-value {font-size:30px;font-weight:800;color:#fff;margin-top:5px;}
.kpi-accent {font-size:11px;color:var(--gold);margin-top:2px;}
[data-testid="stExpander"] {border:1px solid var(--line)!important; border-radius:14px!important; background:rgba(13,24,38,.72)!important;}
[data-testid="stDataFrame"] {border:1px solid var(--line);border-radius:14px;overflow:hidden;background:#0b1624;}
[data-testid="stDataFrame"] * {font-size:13px;}
.stTabs [data-baseweb="tab-list"] {gap:7px;background:rgba(13,24,38,.65);padding:7px;border:1px solid var(--line);border-radius:14px;}
.stTabs [data-baseweb="tab"] {height:42px;border-radius:10px;padding:0 14px;color:#aebacd;background:transparent;}
.stTabs [aria-selected="true"] {background:linear-gradient(135deg,rgba(95,210,255,.15),rgba(213,173,93,.10))!important;color:#fff!important;border:1px solid rgba(95,210,255,.18)!important;}
div[data-testid="stMetric"] {background:linear-gradient(145deg,#101e2f,#0b1624);border:1px solid var(--line);padding:14px 16px;border-radius:15px;box-shadow:0 10px 25px rgba(0,0,0,.18);}
[data-testid="stMetricLabel"] {color:var(--muted)!important;}
[data-testid="stMetricValue"] {color:#fff!important;}
[data-testid="stAlert"] {border-radius:13px;border:1px solid var(--line);}
.stButton>button {border-radius:11px;border:1px solid rgba(95,210,255,.26);background:linear-gradient(135deg,#12263b,#0e1f32);color:#eaf8ff;font-weight:700;}
.stButton>button:hover {border-color:rgba(213,173,93,.55);color:#fff;}
hr {border-color:var(--line)!important;}
.pro-section-title {font-size:20px;font-weight:800;color:#f5f8fc;margin:10px 0 8px;}
.pro-note {color:var(--muted);font-size:12px;}
@media(max-width:900px){.kpi-grid{grid-template-columns:repeat(2,1fr)}.pro-title{font-size:28px}}
</style>
...
st.markdown(f"""
<div class="pro-hero">
  <div class="pro-kicker">Quant · Signal · Warrant Intelligence</div>
  <div class="pro-title">📡 台股權證雷達 <span style="color:#d5ad5d">PRO</span></div>
  <div class="pro-sub">全市場策略掃描 × 盤中訊號 × 權證精選｜把資料、風險與機會集中在同一個交易儀表板</div>
  <div class="pro-badge">{APP_VERSION} · LIVE MARKET WORKSPACE</div>
</div>
""", unsafe_allow_html=True)
...
_k1=len(ranking)
_k2=int((pd.to_numeric(valid["score"], errors="coerce").fillna(0) >= 80).sum())
_k3=int((valid["setup"] == "事件型反彈").sum())
_k4=int(valid["setup"].isin(["趨勢回檔","突破型"]).sum())
st.markdown(f"""
<div class="kpi-grid">
  <div class="kpi-card"><div class="kpi-label">TODAY CANDIDATES</div><div class="kpi-value">{_k1}</div><div class="kpi-accent">今日候選</div></div>
  <div class="kpi-card"><div class="kpi-label">HIGH CONVICTION</div><div class="kpi-value">{_k2}</div><div class="kpi-accent">80 分以上</div></div>
  <div class="kpi-card"><div class="kpi-label">EVENT REVERSAL</div><div class="kpi-value">{_k3}</div><div class="kpi-accent">事件型反彈</div></div>
  <div class="kpi-card"><div class="kpi-label">TREND SETUPS</div><div class="kpi-value">{_k4}</div><div class="kpi-accent">趨勢回檔 / 突破</div></div>
</div>
""", unsafe_allow_html=True)
...
with TAB_TODAY:
    st.markdown('<div class="pro-section-title">今日推薦 · Top Signals</div>', unsafe_allow_html=True)
    ...
    if top_codes:
        ...
        with left:
            ...
            fig.update_layout(height=420, margin=dict(l=10,r=10,t=35,b=10), xaxis_rangeslider_visible=False, paper_bgcolor="#0b1624", plot_bgcolor="#0b1624", font=dict(color="#c8d5e6"), xaxis=dict(gridcolor="rgba(255,255,255,.05)"), yaxis=dict(gridcolor="rgba(255,255,255,.05)"), legend=dict(bgcolor="rgba(0,0,0,0)"))
