...
st.set_page_config(page_title="個人版權證雷達 V7.2.6", page_icon="📡", layout="wide")
...
def stock_ranking(watchlist, hist_map=None):
...
def dynamic_ranking(pool: pd.DataFrame, hist_map=None, max_workers: int = 16):
    """V7.2.6：深度評分改為平行化；若已預載 hist_map 就完全不再逐檔抓歷史。"""
    base_cols = [
    ...
    def _score_one(r):
        ...
        if not code:
            return None
        try:
            h = hist_map.get(code) if isinstance(hist_map, dict) and code in hist_map else load_hist(code)
            sc = stock_score(h)
            row = {
                "code": code, "name": r.get("name", ""),
                "活躍權證數": r.get("活躍權證數", 0),
                "現股成交量(張)": r.get("volume_lots", 0),
                "現股成交金額": r.get("turnover", 0),
            }
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = [ex.submit(_score_one, r) for r in records]
...
def preload_histories_parallel(stock_pool: pd.DataFrame, max_workers: int = 16):
    """V7.2.6：全模組共用同一批歷史資料；首次啟動只抓一次並平行化。"""
...
def build_overheat_ranking(stock_pool, max_scan=120, hist_map=None):
    ...
    for _,rr in stock_pool.head(max_scan).iterrows():
        ...
        try:
            h = hist_map.get(code) if isinstance(hist_map, dict) and code in hist_map else load_hist(code)
            oh = overheat_score_v614(h)
        except Exception as e:
            oh = {"過熱分數":0,"過熱等級":"⚪ 資料錯誤","過熱原因":str(e),"反轉風險":0}
        rows.append({"code":code,"name":rr.get("name",""),**oh})
...
@st.cache_data(ttl=300, show_spinner=False)
def build_kd_decline_ranking(stock_records, expected_trade_date="", hist_map=None):
    """V7.2.6：先篩今日下跌股；若歷史已統一預載，直接共用，不再重抓。"""
    rows = []
    stale = 0
    records = []
    ...
    def _calc_one(rr):
        code = str(rr.get("code","")).strip()
        try:
            h = hist_map.get(code) if isinstance(hist_map, dict) and code in hist_map else kd_hist_base(code)
...
APP_VERSION = "V7.2.6"
APP_BUILD = "首次啟動加速・全模組共用歷史快取・平行深度評分版"
...
# V7.2.6：先一次建立所有需要歷史資料的股票集合，再「只抓一次」。
if scan_mode:
...
_kd_down_codes = []
for _rr in _kd_records:
    _c = str(_rr.get("code", "")).strip()
    _chg = pd.to_numeric(_rr.get("change"), errors="coerce")
    if re.fullmatch(r"\d{4}", _c) and pd.notna(_chg) and float(_chg) < 0:
        _kd_down_codes.append(_c)

_union_codes = []
if pool is not None and not pool.empty:
    _union_codes += pool["code"].astype(str).tolist()
if event_pool is not None and not event_pool.empty:
    _union_codes += event_pool["code"].astype(str).tolist()
_union_codes += [str(c) for c in watchlist]
_union_codes += _kd_down_codes
_union_codes = list(dict.fromkeys([c for c in _union_codes if re.fullmatch(r"\d{4}", str(c))]))
_union_pool = pd.DataFrame({"code": _union_codes})

with st.spinner(f"首次加速載入：共用歷史資料 {_union_pool.shape[0]} 檔（一次抓取、16路平行）…"):
    _shared_hist_map = preload_histories_parallel(_union_pool, max_workers=16)

if scan_mode:
    with st.spinner(f"平行深度評分 {len(pool)} 檔候選股…"):
        ranking = dynamic_ranking(pool, hist_map=_shared_hist_map, max_workers=16)
    ...
    with st.spinner("更新固定關注股…"):
        ranking = stock_ranking(watchlist, hist_map=_shared_hist_map)
...
with st.spinner("計算全市場事件型機會…"):
    event_ranking = event_dynamic_ranking(event_pool, hist_map=_shared_hist_map)

with st.spinner("計算市場過熱股票…"):
    overheat_ranking = build_overheat_ranking(event_pool, max_scan=120, hist_map=_shared_hist_map)

with st.spinner("高速掃描連跌＋3K-2D（直接共用已載入歷史資料）…"):
    kd_decline_ranking = build_kd_decline_ranking(_kd_records, EXPECTED_TRADE_DATE, hist_map=_shared_hist_map)

# 固定關注股另外計算，不受每日 TOP 排名限制
with st.spinner("更新我的關注股…"):
    watch_ranking = stock_ranking(watchlist, hist_map=_shared_hist_map)
